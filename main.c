...
#define SAFETY_COMMANDS_COUNT 300
#define SAFETY_LINES_COUNT 10000
#define MEMORY_LIMIT 1000000000
...
void free_text(struct line* text)
{
	...
	while (text != NULL)
	{
		...
		if(toDelete->content!=NULL)
			charCount -= strlen(toDelete->content);
...
struct line* copy_text(struct line* destination, struct line* source)
{
	...
	while (source->content != NULL)
	{
		...
		charCount += strlen(source->content) + 2;
...
void change_lines(int start, int end)
{
	...
	*/
	...
	if (commandsCount > SAFETY_COMMANDS_COUNT && onlyReads && stepsBackInHistory == 0 && start == currentLineCount + 1)
		current = currentTextEnd;
	else
	{
		while (start != 1 && current->content != NULL)
		{
...
void delete_lines(int start, int end)
{
	...
	if (start == 1)
	{
		while (start <= end && current->content != NULL)
		{
			...
			currentText = current;
			free(toDelete->content);
			charCount -= strlen(toDelete->content) + 1;
			free(toDelete);
			start++;
			currentLineCount--;
		}
