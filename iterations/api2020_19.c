...
int main()
{
	...
	DATA *content = (DATA*) malloc(80* sizeof(DATA));//Total number of operations of c and d that can be performed on data
	for ( int i = 0 ; i < 80 ; i++ )
	{
		content[i].line = (char**) malloc(80*sizeof(char*));
		for ( int j = 0 ; j < 80 ; j++ )
		{
			content[i].line[j] = (char*) malloc(200*sizeof(char));
		}
	}
